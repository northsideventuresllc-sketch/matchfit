...
export function TrainerDashboardAppHeader(props: Props) {
  ...
  async function toggleRead(n: NotifRow) {
    ...
    const res = await fetch(`/api/trainer/notifications/${encodeURIComponent(n.id)}/read`, {
      method: "PATCH",
    ...
    if (res.ok && typeof data.unreadCount === "number") {
      ...
      setNotifs((list) => list?.map((x) => (x.id === n.id ? { ...x, read: nextRead } : x)) ?? null);
      router.refresh();
    }
  }
  ...
  return (
    <header className="mb-8 flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
        <Link
          href={fitHubHref}
          className="rounded-xl border border-white/15 bg-[#0E1016]/80 px-3 py-2 text-[10px] font-black tracking-[0.12em] text-white/90 outline-none ring-[#FF7E00]/30 transition hover:border-[#FF7E00]/35 hover:bg-white/[0.05] focus-visible:ring-2 sm:px-4 sm:text-xs"
        >
          FITHUB
        </Link>
        <Link
                    ...
                    {unreadDropdownItems.map((n) => (
                      <li key={n.id} className="flex gap-2 px-3 py-3">
                        ...
                        <div className="min-w-0 flex-1">
                          {n.linkHref ? (
                            <Link href={n.linkHref} className="block text-left" onClick={() => setNotifOpen(false)}>
        ...
        <div className="relative" ref={wrapRef}>
          ...
          {menuOpen ? (
            ...
            >
              ...
              <Link
                role="menuitem"
                href="/trainer/dashboard/billing"
                className="block px-4 py-3 text-sm font-semibold text-white/90 transition hover:bg-white/[0.06]"
                onClick={() => setMenuOpen(false)}
              >
                Billing Settings
