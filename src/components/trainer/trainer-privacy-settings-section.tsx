...
import type { TrainerOptionalProfileVisibility } from "@/lib/optional-profile-visibility";
...
export function TrainerPrivacySettingsSection() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [ok, setOk] = useState<string | null>(null);
  const [v, setV] = useState<TrainerOptionalProfileVisibility>({});
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/trainer/settings/privacy");
      const data = (await res.json()) as {
        visibility?: TrainerOptionalProfileVisibility;
  ...
  async function savePatch(patch: TrainerOptionalProfileVisibility) {
    ...
    try {
      ...
      const data = (await res.json()) as { visibility?: TrainerOptionalProfileVisibility; error?: string };
      if (!res.ok) {
  ...
  const vis = {
    pronouns: v.showPronouns !== false,
    ethnicity: v.showEthnicity !== false,
    gender: v.showGenderIdentity !== false,
    languages: v.showLanguagesSpoken !== false,
  };

  function scopeSummary(b: BlockedProfileRow): string {
    const bits: string[] = [];
    if (b.kind === "client") {
      if (b.hideClientFromTrainerDiscover) bits.push("Discover");
      if (b.hideTrainerFromClientMatchFeed || b.hideTrainerFromClientFithub) bits.push("Also Hidden on Their Side");
      if (b.blockDirectChat) bits.push("Messages");
    } else {
      if (b.hideBlockedTrainerFromViewerTrainerFithub) bits.push("FitHub");
    }
  ...
  async function deleteAccount() {
    ...
    setDelBusy(true);
    try {
      const res = await fetch("/api/trainer/settings/account/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      window.location.href = "/trainer/dashboard/login";
    } catch {
    ...
    >
      <div className="space-y-4">
        {privacyAcceptedAt ? (
          ...
          <p className="text-xs text-amber-200/80">No privacy acceptance timestamp on file for this account.</p>
        )}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-3">
              <Toggle
                label="Show Pronouns"
                description="Displayed on your public coach profile when you have pronouns saved."
                checked={vis.pronouns}
                disabled={saving}
                onChange={(checked) => {
                  setV((prev) => ({ ...prev, showPronouns: checked }));
                  void savePatch({ showPronouns: checked });
                }}
              />
              <Toggle
                label="Show Ethnicity"
                description="Optional demographic line on your public profile."
                checked={vis.ethnicity}
                disabled={saving}
                onChange={(checked) => {
                  setV((prev) => ({ ...prev, showEthnicity: checked }));
                  void savePatch({ showEthnicity: checked });
                }}
              />
              <Toggle
                label="Show Gender Identity"
                description="Optional line on your public profile."
                checked={vis.gender}
                disabled={saving}
                onChange={(checked) => {
                  setV((prev) => ({ ...prev, showGenderIdentity: checked }));
                  void savePatch({ showGenderIdentity: checked });
                }}
              />
              <Toggle
                label="Show Languages Spoken"
                description="Lists languages you coach in on your public profile."
                checked={vis.languages}
                disabled={saving}
                onChange={(checked) => {
                  setV((prev) => ({ ...prev, showLanguagesSpoken: checked }));
                  void savePatch({ showLanguagesSpoken: checked });
                }}
              />
            </div>
            ...
            {blockedProfiles.length > 0 ? (
              <div className="mt-6 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/40">People You Limited</p>
                <ul className="mt-3 space-y-3">
                  {blockedProfiles.map((b) => (
                    <li
                      key={b.id}
                      className="flex flex-col gap-2 rounded-lg border border-white/[0.06] bg-[#0B0C0F]/80 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-semibold text-white/90">{b.displayName}</p>
                        <p className="truncate text-xs text-white/45">
                          @{b.username}
                          <span className="ml-2 text-white/30">({b.kind === "trainer" ? "Coach" : "Client"})</span>
                        </p>
