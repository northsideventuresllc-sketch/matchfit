...
const n = PLACEHOLDER_TRAINERS.length;
...
export function FeaturedTrainersCarousel() {
  ...
  const active = PLACEHOLDER_TRAINERS[index];
    ...
    >
      <div className="text-center">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-pretty text-sm leading-relaxed text-white/55 sm:text-base">
          Preview local talent on Match Fit. These are placeholder profiles—real
          matches will reflect your goals, schedule, and training style.
        </p>
      </div>
      ...
      <div className="mx-auto mt-8 max-w-lg">
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-[#12151C]/90 px-3 pb-8 pt-8 shadow-[0_34px_90px_-48px_rgba(227,43,43,0.55)] backdrop-blur-xl sm:px-4 sm:pb-10 sm:pt-10">
          ...
          >
            <div
              className={`relative flex h-36 w-36 items-center justify-center rounded-[2rem] shadow-[0_24px_60px_-28px_rgba(0,0,0,0.85)] ring-2 ring-white/10 sm:h-40 sm:w-40 ${active.portraitClass}`}
            >
              <span className="select-none text-4xl font-black tracking-tight text-[#0B0C0F]/90 sm:text-[2.65rem]">
                {active.initials}
              </span>
            </div>
            <p className="mt-8 text-lg font-bold text-white sm:text-xl">{active.name}</p>
