...
export async function POST(req: Request) {
  ...
  if (!claims || claims.accountType !== "trainer") {
    return NextResponse.json({ error: "This link is invalid or has expired." }, { status: 400 });
  }

  await lockAccountForSecurity("trainer", claims.accountId);

  const activeTrainerId = await getSessionTrainerId();
  if (activeTrainerId === claims.accountId) {
