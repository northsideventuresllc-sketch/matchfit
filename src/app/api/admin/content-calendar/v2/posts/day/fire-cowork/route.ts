...
import { z } from "zod";
import { fireCoworkForDay } from "@/lib/content-calendar/content-calendar-cowork-orchestration";
import {
...
export async function POST(req: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Invalid fire Cowork request." }, { status: 400 });

  try {
    await ensureContentCalendarV22Schema();
    const { job, mediaPostCount } = await fireCoworkForDay(parsed.data.postDate);
    return NextResponse.json({ ok: true, jobId: job.id, job, mediaPostCount });
  } catch (e) {
    console.error("[content-calendar v2 fire cowork]", e);
    return NextResponse.json(
      { error: formatUserFacingError(e, "Could not fire Cowork media job.") },
