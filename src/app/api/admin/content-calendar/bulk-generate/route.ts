...
import { CONTENT_CALENDAR_BULK_MAX_COUNT, CONTENT_CALENDAR_GROUPS } from "@/lib/content-calendar/constants";
import { getContentCalendarAiStatusAsync } from "@/lib/content-calendar/content-calendar-ai";
import { requireAdminSession } from "@/lib/require-admin";

const bulkSchema = z.object({
  count: z.number().int().min(1).max(CONTENT_CALENDAR_BULK_MAX_COUNT),
  scheduled: z.boolean(),
  targetGroups: z.array(z.enum(CONTENT_CALENDAR_GROUPS)).min(1),
