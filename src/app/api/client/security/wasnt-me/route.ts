...
export async function POST(req: Request) {
  ...
  if (!claims || claims.accountType !== "client") {
    return NextResponse.json({ error: "This link is invalid or has expired." }, { status: 400 });
  }

  await lockAccountForSecurity("client", claims.accountId);

  const activeClientId = await getSessionClientId();
  if (activeClientId === claims.accountId) {
