import { joinBetaTrainerWaitlist } from "@/lib/beta-waitlist-service";
...
export async function POST(req: Request) {
  ...
  if (!simpleRateLimitAllow(`beta-wl-trainer:${ip}`, 8, 15 * 60 * 1000)) {
  ...
  const res = await joinBetaTrainerWaitlist(parsed.data);
  if ("error" in res) {
