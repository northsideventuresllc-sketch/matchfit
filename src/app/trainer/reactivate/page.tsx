...
function ReactivateContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/trainer/reactivate/auth", {
        method: "POST",
      ...
      navigateWithFullLoad(data.next ?? "/trainer/reactivate/checkout");
    } catch {
    ...
    <main className="relative min-h-dvh overflow-x-hidden bg-[#0B0C0F] text-white antialiased">
      <div className="relative z-10 mx-auto max-w-lg px-5 py-12 sm:px-8">
        ...
        <p className="mt-4 text-sm leading-relaxed text-white/65">
          Independent Pro accounts include a {TRAINER_PLATFORM_TRIAL_DAYS}-day free trial at registration, then{" "}
          {TRAINER_PAYMENT_GRACE_DAYS} days to add a card and start the {trainerPlatformSubscriptionLabel()} subscription.
          After that grace period ends without payment, access is deactivated. Reactivation requires payment — a new free
        ...
        <p className="mt-8 text-center text-xs text-white/40">
          <Link href="/trainer/signup" className="underline-offset-2 hover:text-white/60 hover:underline">
...
export default function TrainerReactivatePage() {
  return (
