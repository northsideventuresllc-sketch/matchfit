...
import { PasswordChangePanel } from "@/app/client/settings/password-change-panel";
import { StayLoggedInPanel } from "@/app/client/settings/stay-logged-in-panel";
import {
  TwoFactorPanel,
  type TwoFactorChannelDTO,
  type TwoFactorPanelRef,
} from "@/app/client/settings/two-factor-panel";
import {
  TrainerProfileSettingsPanel,
...
export function TrainerSettingsPageClient(props: Props) {
  ...
  const runSave = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (stayDirty) {
        const res = await fetch(`${TRAINER_SETTINGS_API}/session`, {
          method: "PATCH",
      ...
      try {
        await fetch(`${TRAINER_SETTINGS_API}/2fa`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      {dirty ? (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#0B0C0F]/95 px-5 py-4 backdrop-blur-md sm:px-8">
          <div className="mx-auto flex max-w-2xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              type="button"
              disabled={footerBusy || twoFaFooterBlocked}
