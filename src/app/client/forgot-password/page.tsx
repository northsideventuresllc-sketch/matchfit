...
export default function ClientForgotPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/client/forgot-password/request", {
        method: "POST",
    ...
    <main className="min-h-dvh bg-[#0B0C0F] px-5 py-12 text-white sm:px-8">
      <div className="mx-auto max-w-md">
        ...
        <p className="mt-2 text-sm leading-relaxed text-white/55">
          Enter the username, email, and date of birth on your account. If everything matches, we&apos;ll email a reset
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4" noValidate>
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="fp-dob" className={labelClass}>Date of birth</label>
              <input
                id="fp-dob"
                type="date"
                value={dateOfBirth}
