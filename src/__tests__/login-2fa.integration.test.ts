...
describe.skipIf(!process.env.TEST_DATABASE_URL?.trim())("login 2FA API", () => {
  ...
  function applyResponseCookiesToJar(res: Response): void {
    ...
    for (const value of setCookieValues) {
      ...
      testCookieJar.set(name, cookieValue);
    }
  }
  ...
  it("issues session after correct code and applies stayLoggedIn=false", async () => {
    ...
    applyResponseCookiesToJar(loginRes);
    ...
    applyResponseCookiesToJar(completeRes);
  ...
  it("locks after 3 wrong codes, then resend + correct code succeeds", async () => {
    ...
    applyResponseCookiesToJar(okRes);
