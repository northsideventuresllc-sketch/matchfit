...
export function buildMediaGenerationPrompt(args: {
  ...
  return [
    ...
    "- WATERMARK CROP FRAME (mandatory): render the real artwork INSET inside the canvas with a clean solid margin band framing the entire edge, sized so the generator's corner AI watermark falls INSIDE that disposable band and not on the artwork. Keep ALL text, logo, subject and CTA within the inset artwork — nothing important in the margin band. Oversize the canvas so that after the band is cropped away the result is still the exact target pixel dimensions above. The band is scaffolding: it is cropped off before publishing and must never appear in the final post.",
