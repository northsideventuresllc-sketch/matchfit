...
type OutreachBulkDeleteInput =
  | { mode: "all" }
  | { mode: "batch"; generationBatchId: string }
  | { mode: "ids"; ids: string[] };

export function buildMassDeleteOutreachWhere(input: OutreachBulkDeleteInput) {
  if (input.mode === "all") {
...
export async function massSoftDeleteOutreachLeads(
  platform: OutreachPlatform,
  input: OutreachBulkDeleteInput,
): Promise<{ deletedCount: number }> {
  const where = buildMassDeleteOutreachWhere(input) as Record<string, unknown>;
