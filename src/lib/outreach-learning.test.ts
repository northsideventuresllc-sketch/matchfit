...
describe("outreach-learning helpers", () => {
  ...
  it("stores edit signals with bounded text length and field metadata", async () => {
    await recordOutreachEditSignal({
      platform: "email",
      leadId: "lead_2",
      field: "emailBody",
  ...
  it("builds a learning summary with outcome counts and recent edits", async () => {
    ...
    const summary = await buildOutreachLearningContext("instagram");
    ...
    expect(mockOutreachLearningSignalFindMany).toHaveBeenCalledTimes(2);
  });
  ...
  it("returns fallback guidance when learning context query fails", async () => {
    ...
    const summary = await buildOutreachLearningContext("facebook");
