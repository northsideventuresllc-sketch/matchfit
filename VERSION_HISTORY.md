...
- **2026-09-14** `2.33.0-beta` → `2.33.1-beta` (**patch** — Security audit fixes: RLS auto-disable removed, RLS added on 20 tables, security headers, login rate limiting, timing-safe secret comparisons)
...
- **2026-09-07** `2.29.7-beta` → `2.29.8-beta` (**patch** — Cache trending-hashtag research (24h) so content generators skip the up-to-120s web search and no longer time out before writing posts)
...
- **2026-09-03** `2.26.9-beta` → `2.26.10-beta` (**patch** — Log AI Vault provider calls to the AXON usage ledger)
...
- **2026-08-19** `2.23.3-beta` → `2.24.0-beta` (**minor** — Ad Tracking HQ: collapsible sections, per-campaign drilldown, RUNBOOK rename, AI ads-analysis copilot)
- **2026-08-13** `2.23.2-beta` → `2.23.3-beta` (**patch** — Enforce JB's daily media generation cap (1 static + 1 carousel + 1 video/day) in the generate-media cron)
- **2026-08-13** `2.23.1-beta` → `2.23.2-beta` (**patch** — Fix content calendar post_date/day_index desync when weekly-generate reassigns a taken slot)
