#!/usr/bin/env node
/**
 * Push NI Brain database password to Vercel production via REST API.
 * Requires VERCEL_TOKEN (personal/team token from vercel.com/account/tokens).
 *
 * Usage:
 *   VERCEL_TOKEN=... NI_BRAIN_DATABASE_PASSWORD=... node scripts/vercel-env-set-ni-brain-database-password.mjs
 */
...
const ENV_KEYS = ["NI_BRAIN_DATABASE_PASSWORD"];
...
function loadValues() {
  ...
  const password = map.get("NI_BRAIN_DATABASE_PASSWORD") ?? "";
  if (!password) {
    console.error("NI_BRAIN_DATABASE_PASSWORD must be set.");
    process.exit(1);
  }
...
async function main() {
  ...
  console.log(`Done (${projectId}). Redeploy production for the env var to take effect.`);
}
