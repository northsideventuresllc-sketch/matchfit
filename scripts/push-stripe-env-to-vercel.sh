...
source .env
set +a
for name in STRIPE_SECRET_KEY NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY STRIPE_WEBHOOK_SECRET NEXT_PUBLIC_TRAINER_BACKGROUND_CHECK_FEE_USD NEXT_PUBLIC_APP_URL; do
  val="${!name:-}"
  if [[ -z "$val" ]]; then
